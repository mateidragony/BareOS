...
void uart_handler(void) {
  ...
  if (code == UART_RX_INTR)      {                        /*  If interrupt was caused by a keypress                                                  */
    ...
    if (tty_in_count == TTY_BUFFLEN) return;
    // while(tty_in_count == TTY_BUFFLEN);

    tty_in[(tty_in_head + tty_in_count) % TTY_BUFFLEN] = ch;
    tty_in_count++;

    if(tty_in_sem_enabled) sem_post(tty_in_sem);
    else tty_in_sem = 0;
    ...
    if(tty_out_sem_enabled) sem_post(tty_out_sem);
    else tty_out_sem = 0;

    uart[UART0_RW_REG] = c == '\r' ? '\n' : c;

    if(tty_out_count == 0) set_uart_interrupt(0);
  }
}
